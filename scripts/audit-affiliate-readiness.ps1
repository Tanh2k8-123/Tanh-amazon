...
$evidenceSourceDocumentedRows = @($evidenceRows | Where-Object { $_.'Evidence status' -eq 'Source documented' })
$evidenceNeedsVerificationRows = @($evidenceRows | Where-Object { $_.'Evidence status' -eq 'Needs official verification' })
$evidenceHoldRows = @($evidenceRows | Where-Object { $_.'Evidence status' -like 'Hold -*' })
...
[pscustomobject]@{
  ...
  EvidenceRowsSourceDocumented = $evidenceSourceDocumentedRows.Count
  EvidenceRowsNeedingVerification = $evidenceNeedsVerificationRows.Count
  EvidenceRowsOnHold = $evidenceHoldRows.Count
